...
class EstadisticasEmociones:
    ...
    def exportarEstadisticas(self, dateFrom, dateTo):
        ...
        pdf = canvas.Canvas(ruta_escritorio + "/Emociones.pdf")
        #pdf = canvas.Canvas("../data/users/" + self.usuario.username + "/Emociones.pdf")
        y = 750
        pdf.drawString(100, y, "Estadísticas de Emociones del usuario " + self.usuario.username + " del " + dateFrom + " al " + dateTo)
        y-=30
...
def generarDatos(username):
    ruta = "TFG/data/users/" + username + "/emociones.txt"
    data = {}
